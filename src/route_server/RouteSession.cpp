...
RouteSession::RouteSession(const std::shared_ptr<TcpConnection>& conn, const std::string& sessionname, int sessionid) : TcpSession(conn), m_sessionname(sessionname), m_id(sessionid) {
    m_lastPackageTime = Timestamp(time(NULL));
    m_interval = 15;
    m_dispatcher.setDefaultCallback(std::bind(&RouteSession::onUnknownMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    m_dispatcher.registerMessageCallback<Msg2Route::LoginRsp>(std::bind(&RouteSession::handleLoginRsp, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void RouteSession::enableHeartbeat() {
    std::shared_ptr<TcpConnection> conn = getConnectionPtr();
    if (conn)
    {
        //每15秒钟检测一下是否有掉线现象
        conn->getLoop()->assertInLoopThread();
        m_TimerId = conn->getLoop()->runEvery(m_interval * 1000 * 1000, std::bind(&RouteSession::checkHeartbeat, this, conn));
    }
}

void RouteSession::disableHeartbeat() {
    std::shared_ptr<TcpConnection> conn = getConnectionPtr();
    if (conn)
    {
        char log[255];
...
void RouteSession::checkHeartbeat(const std::shared_ptr<TcpConnection>& conn) {
    if (!conn) {
        return;
    }
...
// void RouteSession::onRead(const std::shared_ptr<TcpConnection>& conn, Buffer* pBuffer, Timestamp receivTime) {
//     while (true) {
...
// bool RouteSession::process(const std::shared_ptr<TcpConnection>& conn, IM::Msg& msg) {
//     int sid = msg.sid();
//     switch (sid) 
//     {
...
// void RouteSession::handleRegister(const std::shared_ptr<TcpConnection>& conn, IM::Msg& msg) {
//     int cmd = msg.cmd();
//     switch (cmd)
//     {
...
//send kickuser message and notify user other device login
void RouteSession::handleLoginRsp(const std::shared_ptr<TcpConnection>& conn, const std::shared_ptr<Msg2Route::LoginRsp>& rsp, Timestamp t) {
